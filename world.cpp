...
void World::spawnDestructionParticles(int x, int y, int z)
{
    ...
    p.tae.left += tex_width / 2 - tex_width / 4;
    p.tae.right -= tex_width / 2 - tex_width / 4;
    p.tae.top += tex_height / 2 - tex_width / 4;
    p.tae.bottom -= tex_height / 2 - tex_width / 4;

    const auto randMax = [](GLFix max) { return max * (rand() & 0xFF) / 256; };
    ...
    for(int i = 0; i < 4; ++i)
    {
        ...
        p.pos = {aabb.low_x - c->absX(), aabb.low_y - c->absY(), aabb.low_z - c->absZ()};
        p.pos.x += randMax(aabb.high_x - aabb.low_x);
        p.pos.y += randMax(aabb.high_y - aabb.low_y);
        p.pos.z += randMax(aabb.high_z - aabb.low_z);
        p.pos.print();
        c->addParticle(p);
    }
}
