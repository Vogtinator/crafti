...
void CakeRenderer::renderSpecialBlock(const BLOCK_WDATA /*block*/, GLFix x, GLFix y, GLFix z, Chunk &c)
{
    ...
    // Size of cake slice
    const GLFix cake_size = cake_size / 2;
    const GLFix cake_slice_offset = (GLFix(BLOCK_SIZE) - cake_width) * GLFix(0.5f);
    ...
    // Cake Left Side
    c.addUnalignedVertex({x - (cake_slice_offset + cake_size), y, z, cake_sid.left, cake_sid.bottom, TEXTURE_TRANSPARENT});
    c.addUnalignedVertex({x - (cake_slice_offset + cake_size), y + cake_height, z, cake_sid.left, cake_sid.top, TEXTURE_TRANSPARENT});
    c.addUnalignedVertex({x - (cake_slice_offset + cake_size), y + cake_height, z + BLOCK_SIZE, cake_sid.right, cake_sid.top, TEXTURE_TRANSPARENT});
    c.addUnalignedVertex({x - (cake_slice_offset + cake_size), y, z + BLOCK_SIZE, cake_sid.right, cake_sid.bottom, TEXTURE_TRANSPARENT});
